...
async function createUsername(firstName, lastName) {
  ...
  do {
    const randomNum = Math.floor(1000 + Math.random() * 9000); // Menghasilkan angka acak 4 digit
    newUsername = `${firstName.toLowerCase()}${lastName.toLowerCase()}${randomNum}`;
...
export const Register = async (req, res) => {
  ...
  try {
    ...
    res.json({ msg: "Register Berhasil!", username });
  } catch (error) {
    console.log(error);
    res.status(500).json({ msg: "Server error" });
  }
